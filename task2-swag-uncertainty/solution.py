...
class SWAInferenceHandler(object):
    ...
    def __init__(
        ...
        self.theta_i = self._create_weight_copy()
        ...
        self.D = collections.deque(maxlen=self.max_rank_deviation_matrix)
    ...
    def update_swag_statistics(self) -> None:
        ...
        copied_params = {name: param.detach().cpu().clone() for name, param in self.network.named_parameters()}
        ...
        for name, param in copied_params.items():
            ...
            self.theta_i[name] = (self.theta_i[name] * self.n_models_collected + param)/(self.n_models_collected + 1)
            self.theta_squared[name] = (self.theta_squared[name] * self.n_models_collected + param**2) / (self.n_models_collected + 1)
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            Di = {}
            for name, param in copied_params.items():
                Di[name] = (param - self.theta_i[name])
            self.D.append(Di)
    ...
    def fit_swag_model(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        self.network.load_state_dict(torch.load(self.model_dir / "map_weights.pt"))
        ...
        with tqdm.trange(self.swag_training_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                if (epoch + 1) % self.swag_update_interval == 0:
                    self.n_models_collected = (epoch + 1) // self.swag_update_interval
                    self.update_swag_statistics()
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.num_bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for(batch_images, ) in loader:
                batch_preds.append(self.network(batch_images))
        ...
        bma_probabilities = torch.stack(model_predictions, axis=0).mean(dim=0)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            mean_weights = self.theta_i[name]
            std_weights = torch.sqrt(self.theta_squared[name] - self.theta_i[name]**2)
            #assert mean_weights.size() == param.size() and std_weights.size() == param.size()

            # Diagonal part
            diag_term = std_weights * z_diag
            if self.inference_mode == InferenceMode.SWAG_FULL:
                diag_term = diag_term / math.sqrt(2.0)
            sampled_weight = mean_weights + diag_term
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                coeffs = z_full / math.sqrt(2 * (K - 1))
                for k, Di in enumerate(self.D):
                    sampled_weight +=  Di[name] * coeffs[k].item()
