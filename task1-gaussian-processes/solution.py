...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = C(1.0, (1e-3, 1e3)) * RBF(length_scale=1) + C(1.0, (1e-3, 1e3)) * Matern(length_scale=1, nu=1.5) + C(1.0, (1e-3, 1e3)) * Matern(length_scale=1, nu=2.5) + C(1.0, (1e-3, 1e3)) * WhiteKernel(noise_level=1e-3)
        self.models = []
    
        # GMM
        self.num_clusters = 40
        self.gmm = GaussianMixture(n_components=self.num_clusters, random_state=42)
        self.min_p = 0.2
        self.c = 1
               
        self.z_flagged = 2.062    # quantile shift in std units
        self.z_unflagged = 0.0
    ...
    def predict_pollution_concentration(self, test_coordinates: np.ndarray, test_area_flags: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = np.zeros((test_coordinates.shape[0],))
        gp_mean = np.zeros((test_coordinates.shape[0],))
        gp_stddev = np.zeros((test_coordinates.shape[0],))
        
        # assign test points to clusters
        test_cluster_id = self.gmm.predict(test_coordinates)
        
        for i in range(self.num_clusters):
            print(f"Predicting for cluster {i+1}/{self.num_clusters}. Cluster size: {np.sum(test_cluster_id == i)}")
            
            cluster_indices = np.where(test_cluster_id == i)[0]
            
            if len(cluster_indices) == 0 or i >= len(self.models):
                assert 1 == 0
            
            cluster_test_coords = test_coordinates[cluster_indices]
            
            model = self.models[i]
            
            cluster_gp_mean, cluster_gp_stddev = model.predict(cluster_test_coords, return_std=True)
            
            gp_mean[cluster_indices] = cluster_gp_mean
            gp_stddev[cluster_indices] = cluster_gp_stddev
            z_vec = np.where(test_area_flags[cluster_indices], self.z_flagged, self.z_unflagged)
            predictions[cluster_indices] = cluster_gp_mean + z_vec * cluster_gp_stddev
        
        return predictions, gp_mean, gp_stddev
    ...
    def fit_model_on_training_data(self, train_targets: np.ndarray, train_coordinates: np.ndarray, train_area_flags: np.ndarray):
        ...
        # comment when running full model
        # train_coordinates = train_coordinates[:MAX_SAMPLES]
        # train_targets = train_targets[:MAX_SAMPLES]
        
        self.gmm.fit(train_coordinates)
        probs_train = self.gmm.predict_proba(train_coordinates)
        
        for i in range(self.num_clusters):
            print(f"Fitting model for cluster {i+1}/{self.num_clusters}. Cluster size: {np.sum(probs_train[:, i] > self.min_p)}")
            cluster_indices = np.where(probs_train[:, i] > self.min_p)[0]
            if len(cluster_indices) == 0:
                continue
            
            cluster_train_coords = train_coordinates[cluster_indices]
            cluster_train_targets = train_targets[cluster_indices]
            ...
            model.fit(cluster_train_coords, cluster_train_targets)
...
def main():
    ...
    val_x, val_area_flags, _, _ = get_city_area_data(val_x, test_x)
